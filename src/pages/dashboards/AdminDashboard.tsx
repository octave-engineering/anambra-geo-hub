...
import {
  ...
  Activity, Server, Key, LogOut, UserCog, FileCheck
} from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const handleRejectUser = async (userId: number) => {
    if (!confirm("Are you sure you want to reject this user request? This action cannot be undone.")) {
      return;
    }
    ...
    setPendingError(null);
    try {
      const response = await authenticatedFetch(`${API_BASE}/admin/users/${userId}/reject`, {
        method: "DELETE",
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok || data.ok === false) {
        throw new Error(data.error || `Failed to reject user (${response.status})`);
      }
      ...
      setRejectingId((prev) => (prev === userId ? null : prev));
    }
  };
          ...
          <TabsList className="grid w-full grid-cols-7 lg:w-auto">
            ...
            <TabsTrigger value="pending-approvals">
              ...
              {pendingUsers.length > 0 && (
                <Badge className="ml-2 bg-red-500 text-white">{pendingUsers.length}</Badge>
              )}
            </TabsTrigger>
          ...
          <TabsContent value="pending-approvals" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Review and approve access requests for admin and partner roles
                </CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-muted-foreground">
                            Requested role: <span className="font-medium">{user.role}</span>
                          </span>
              ...
              <CardContent>
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-muted-foreground">
                    View all users, modify roles, reset passwords, and manage permissions
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-center py-12">
                  ...
                  <p className="text-muted-foreground mb-4">System configuration coming soon</p>
                  <p className="text-sm text-muted-foreground">
                    Manage API endpoints, database connections, email settings, and more
                  </p>
                </div>
              </CardContent>
            </Card>
          ...
          <TabsContent value="security-audit" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Review security logs, authentication attempts, and system access
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="text-center py-12">
                  <Shield className="h-16 w-16 text-muted-foreground mx-auto mb-4" />
                  <p className="text-muted-foreground mb-4">Security audit features coming soon</p>
                  <p className="text-sm text-muted-foreground">
                    View login attempts, access logs, permission changes, and security reports
                  </p>
                </div>
              </CardContent>
            </Card>
          ...
          <TabsContent value="system-alerts" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Monitor system health, warnings, and important notifications
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
